...
- An instant upload moves no bytes and leaves the storage quota of `about` untouched, but it draws 15 % of the file's size from the monthly *upload* transfer allowance (`getTransferQuota`); an offline download draws its full size from the *offline* allowance. Per byte of content the instant path costs about six times more of its allowance, so it is the fast path for a file or two, not the cheap path for a pack. The signed link it yields outlives the file object, permanent deletion included.
